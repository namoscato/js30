...
(() => {
    ...
    // eslint-disable-next-line max-len
    // Array.prototype.slice.apply(document.querySelectorAll('.mw-category a')).map(element => element.textContent).filter(text => text.includes('de'));
    ...
    console.log(data.reduce((hash, item) => {
        if (typeof hash[item] === 'undefined') {
            hash[item] = 0; // eslint-disable-line no-param-reassign
        }

        hash[item] += 1; // eslint-disable-line no-param-reassign
